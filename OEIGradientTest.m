classdef OEIGradientTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function verifyOEIGradient(testCase)
            ...
            OEIGradient_param = comp.getParameter("OEIGradient");                  % parameter object

            % Convert the Range parameter's stored string into a numeric value.
            % (sscanf reads the number out of the text string.)
            OEIGradient_param_val = sscanf(OEIGradient_param.Value, '%f', 1);  % read

            % Test assertion: check that the PAX is greater than 9.
            % If not, the test fails.
            testCase.verifyGreaterThan(OEIGradient_param_val, 2);  
        end
    end
end
