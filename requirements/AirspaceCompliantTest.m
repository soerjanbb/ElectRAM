classdef AirspaceCompliantTest < matlab.unittest.TestCase
    ...
    methods (Test)
        ...
        function verifyAirspaceCompliant(testCase)
            ...
            AirspaceCompliant_param = comp.getParameter("AirspaceCompliant");                  % parameter object

            % Convert the Range parameter's stored string into a numeric value.
            % (sscanf reads the number out of the text string.)
            AirspaceCompliant_param_val = sscanf(AirspaceCompliant_param.Value, '%f', 1);  % read

            % Test assertion: check that the PAX is greater than 9.
            % If not, the test fails.
            testCase.verifyEqual(AirspaceCompliant_param_val, 1);  
        end
    end
end
